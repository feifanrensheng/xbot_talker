...
int main(int argc,char** argv){
    ...
    ros::ServiceServer service = nodeHandle.advertiseService("request_talker", request);

    nodeHandle.param("/xbot_talker/base_path",basePath, string("/home/zn/catkin_test/src/xbot_talker"));
//    ROS_ERROR("%s\n",basePath.c_str());
	         ...
	         xbot_talker::request_talker::Response &res)
{
	if (req.request_talker == 1)
	{
		...
		ROS_INFO("response: %d", (int)res.response);
