...
bool request(xbot_talker::request_talker::Request &req,
	         xbot_talker::request_talker::Response &res);

bool request_play(xbot_talker::request_play::Request &req,
                  xbot_talker::request_play::Response &res);
...
int main(int argc,char** argv){
    ...
    ros::ServiceServer service_talker = nodeHandle.advertiseService("request_talker", request);
    ros::ServiceServer service_play = nodeHandle.advertiseService("request_play", request_play);
...
bool request(xbot_talker::request_talker::Request &req,
	         xbot_talker::request_talker::Response &res)
{
	if (req.request_talker == 1)
	{
		n = 0;
		res.response = "success";
		ROS_INFO("request: %d", (int)req.request_talker);
		ROS_INFO("response: %s", res.response);
		onPlayFinished(REQUEST_GREET_VISITOR, "chat");
	}
}
                  ...
                  xbot_talker::request_play::Response &res)
{
    if (req.choose == 1)
    {
        res.feedback = "play wav";
        string audiofile = req.path;
        talker.play((char*)audiofile.c_str(),REQUEST_SIMPLE_PLAY,&onPlayFinished);
    }
    if (req.choose == 2)
    {
        res.feedback = "play text";
...
//    cout<<"onGetFaceResult  ---- tid: "<<this_thread::get_id()<<endl;
    ...
    if(!isVerifying){
	    ...
	    onPlayFinished(REQUEST_VERIFY_COMPLETE, "chat");
            isVerifying =false;
        }
    }
...
//    cout<<"-----------------------  onPlayFinished  ---- tid: "<<this_thread::get_id()<<endl;
    ...
    switch(code){
        ...
        case REQUEST_GREET_VISITOR:
        {
            ...
            talker.text_to_speech(text.c_str(),file.c_str(),REQUEST_SIMPLE_PLAY,onPlayFinished); 	//by fw
            isPlayingAudio = false;
            condition_playing_audio.notify_one();
            isChatting = true;
            condition_chat.notify_one();
